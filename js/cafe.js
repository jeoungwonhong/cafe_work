$(function(){
    ...
    $('.search-select').click(function(e){
     ...
     //slide show
     let slide = setInterval( mySilde, 10000 );
     $('.next').click(function(){
      clearInterval(slide);
      mySilde();
      slide = setInterval( mySilde, 10000 );
     });
     ...
     jQuery.ajax({
      ...
      success: function(data){
         ...
         for(let i = 0; i < data.cafelist.length; i++){
            ...
            list += '<h1>' + data.cafelist[i].num + '</h1><p class="ellipse">' + data.cafelist[i].content + '</p></div>';
            list += '<div class="cfe d-flex"><p class="ellipse">' + data.cafelist[i].cafename + '</p>';
